...
const labLogo = document.querySelector('.lab-menu-img')
...
function showHeader(){
    ...
    if ((window.pageYOffset > 0) && (window.pageYOffset < rect - tamanhoHeader)) {
        ...
        labLogo.classList.add("fixed-lab-menu-img");
    } 
    
    else {
        header.classList.remove("fixed-header");
        header.classList.remove("fixed-background");
        labLogo.classList.remove("fixed-lab-menu-img");
...
if(targetAnime.length) { //verifica se tem item no targetAnime, caso não, ele não execute a função.
